...
void setHueOn(int lightNum, boolean on) {
  if (client.connect(hueBridgeIP, hueBridgePort)) {
    while (client.connected()) {
      ...
      client.println();
      if (on) client.println("{\"on\":true}");
      else client.println("{\"on\":false}");
      Serial.print("Sent command: light #");
    ...
    Serial.println("setHueOn() - Command failed");
  }
}
